...
void main() {
  group('ThumbnailMapper', () {
    test('should correctly map a JSON to a Thumbnail', () {
      final json = {'path': 'http://example.com/image', 'extension': 'jpg'};

      final result = ThumbnailMapper.fromJson(json);

      expect(result, isA<Thumbnail>());
      expect(result.path, 'http://example.com/image');
      expect(result.extension, 'jpg');
    });

    test('should handle different file extensions', () {
      final json = {'path': 'http://example.com/image', 'extension': 'png'};
    ...
    test('should throw an exception if the JSON is missing required fields',
        () {
      final invalidJson = {'path': 'http://example.com/image'};
    ...
    test('should handle URLs with query parameters', () {
      final json = {
        'path': 'http://example.com/image?size=large',
        'extension': 'webp'
      };

      final result = ThumbnailMapper.fromJson(json);

      expect(result.path, 'http://example.com/image?size=large');
